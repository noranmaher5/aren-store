...
export default function ProductCard({ product }) {
  ...
  <div className="aren-product-body"><span className="aren-product-label">{product.productType === 'service' ? 'خدمة رقمية' : promotionName || catalogCategory?.shortName || 'منتج رقمي'}</span><Link to={`/products/${product._id}`} className="aren-product-name"><h2>{product.name}</h2></Link><div className="aren-rating"><span>★★★★★</span><small>{Number(product.rating?.average || 0).toFixed(1)} ({product.rating?.count || 0})</small></div><div className="aren-product-bottom"><div>{hasOptions && <small style={{ display: 'block', color: 'rgba(255,255,255,.5)', fontSize: 10 }}>يبدأ من</small>}<strong>{format(product.price || 0)}</strong>{product.originalPrice > product.price && <del>{format(product.originalPrice)}</del>}</div><button className="aren-add-button" disabled={out} onClick={event => { event.preventDefault(); event.stopPropagation(); hasOptions ? navigate(`/products/${product._id}`) : addItem(product); }}>{out ? 'نفد المخزون' : quoteOnly ? 'تواصل واتساب' : hasOptions ? 'اختر الباقة' : 'أضف +'}</button></div></div>
  </article>;
}
